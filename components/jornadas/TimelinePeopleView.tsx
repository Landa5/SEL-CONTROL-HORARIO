...
export default function TimelinePeopleView({ jornadas, date }: TimelinePeopleViewProps) {
    ...
    sortedRoles.forEach(role => {
        roleGroups[role].sort((a, b) => a.empleado.nombre.localeCompare(b.empleado.nombre));
    });
                            ...
                            {sortedRoles.map(role => (
                                <div key={role} className="space-y-2">
                                    <div className="flex items-center gap-2 px-2 pb-1 border-b border-gray-100">
                                        ...
                                        <div className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full font-bold">{roleGroups[role].length}</div>
                                    </div>

                                    <div className="space-y-1">
                                        {roleGroups[role].map(jor => {
                                            ...
                                            return (
                                                <div key={jor.id} className="flex items-center group hover:bg-gray-50 rounded-lg py-1.5 pl-2 transition-colors relative">
                                                    ...
                                                    <div className="w-44 shrink-0 pr-4 flex items-center gap-2 relative z-10">
                                                        ...
                                                        <div className="min-w-0">
                                                            <div className="font-bold text-gray-700 text-xs truncate w-28">{jor.empleado.nombre}</div>
                                                            <div className="text-[10px] text-gray-400 font-mono flex gap-1">
                                                                {format(start, 'HH:mm')} - {jor.horaSalida ? format(end, 'HH:mm') : '...'}
                                                            </div>
                                                        </div>
                                                    </div>
